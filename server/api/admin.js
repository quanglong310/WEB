...
router.get('/token', JwtUtil.checkToken, function (req, res) {
  ...
  res.json({ success: true, message: 'Sai token', token: token });
});
...
router.get('/customers/sendmail/:id', JwtUtil.checkToken, async function (req, res) {
  ...
  if (cust) {
    ...
    if (send) {
      res.json({ success: true, message: 'Đã gửi thông tin kích hoạt tài khoản tới người dùng!' });
    } else {
      res.json({ success: false, message: 'Gửi mail thất bại!' });
    }
  } else {
    res.json({ success: false, message: 'Không tồn tại người dùng!' });
  }
});
